...
import { parentService, type ParentReport } from '../../services/parentService';
...
const StudentReports: React.FC<StudentReportsProps> = ({ 
  ...
  const [filteredReports, setFilteredReports] = useState<ParentReport[]>([]);
  ...
  const loadReports = async () => {
    try {
      setLoading(true);
      const reportsData = await parentService.getStudentReports(studentId);
  ...
  const filterReports = () => {
    ...
    if (searchTerm) {
      filtered = filtered.filter(report => 
        ...
        report.academicYear.includes(searchTerm) ||
        report.overallGrade.toLowerCase().includes(searchTerm.toLowerCase())
      );
    }
  ...
  const handleDownloadReport = async (report: ParentReport) => {
    try {
      ...
      if (report.subjectReports && report.subjectReports.length > 0) {
        ...
        report.subjectReports.forEach(subject => {
          const totalMark = subject.courseworkMark && subject.examMark 
            ...
            : subject.finalMark;
          const grade = totalMark ? (totalMark >= 90 ? 'A' : totalMark >= 80 ? 'B' : totalMark >= 70 ? 'C' : totalMark >= 60 ? 'D' : 'F') : 'N/A';
  ...
  const getUniqueTerms = () => {
    return [...new Set(reports.map(report => report.term))];
  };

  const getUniqueYears = () => {
    return [...new Set(reports.map(report => report.academicYear))].sort((a, b) => b.localeCompare(a));
  };
      ...
      <Card className={`p-6 ${className}`}>
        <div className="text-center">
          ...
          <p className="text-gray-600 mb-4">
            Please settle all outstanding fees to access student reports.
          </p>
          <div className="p-4 bg-red-50 border border-red-200 rounded-lg">
            <p className="text-sm text-red-700">
              <AlertCircle className="h-4 w-4 inline mr-1" />
              Outstanding fees must be paid before reports can be viewed.
            </p>
          </div>
        </div>
      </Card>
      ...
      <Card className={`p-6 ${className}`}>
        <div className="flex items-center justify-between mb-6">
          ...
          <div className="flex items-center space-x-2">
            ...
            <span className="text-sm text-green-600 font-medium">Fees Paid</span>
          </div>
        </div>
